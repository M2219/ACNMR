...
: impl_(std::make_unique<AckermannDrivePrivate>())
{
}
...
void AckermannDrive::Load(gazebo::physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  impl_->model_ = _model;
  ...
  impl_->joints_[AckermannDrivePrivate::STEER_WHEEL] =
    _model->GetJoint(steering_wheel_joint);
  if (!impl_->joints_[AckermannDrivePrivate::STEER_WHEEL]) {
    RCLCPP_WARN(
  ...
  impl_->joints_[AckermannDrivePrivate::FRONT_RIGHT] = _model->GetJoint(front_right_joint);
  if (!impl_->joints_[AckermannDrivePrivate::FRONT_RIGHT]) {
    RCLCPP_ERROR(
      impl_->ros_node_->get_logger(),
  ...
  impl_->joints_[AckermannDrivePrivate::FRONT_LEFT] = _model->GetJoint(front_left_joint);
  if (!impl_->joints_[AckermannDrivePrivate::FRONT_LEFT]) {
    RCLCPP_ERROR(
      impl_->ros_node_->get_logger(),
  ...
  impl_->joints_[AckermannDrivePrivate::REAR_RIGHT] = _model->GetJoint(rear_right_joint);
  if (!impl_->joints_[AckermannDrivePrivate::REAR_RIGHT]) {
    RCLCPP_ERROR(
      impl_->ros_node_->get_logger(),
  ...
  impl_->joints_[AckermannDrivePrivate::REAR_LEFT] = _model->GetJoint(rear_left_joint);
  if (!impl_->joints_[AckermannDrivePrivate::REAR_LEFT]) {
    RCLCPP_ERROR(
      impl_->ros_node_->get_logger(),
  ...
  impl_->joints_[AckermannDrivePrivate::STEER_RIGHT] =
    _model->GetJoint(right_steering_joint);
  if (!impl_->joints_[AckermannDrivePrivate::STEER_RIGHT]) {
    RCLCPP_ERROR(
      impl_->ros_node_->get_logger(),
  ...
  impl_->joints_[AckermannDrivePrivate::STEER_LEFT] =
    _model->GetJoint(left_steering_joint);
  if (!impl_->joints_[AckermannDrivePrivate::STEER_LEFT]) {
    RCLCPP_ERROR(
      impl_->ros_node_->get_logger(),
  ...
  impl_->wheel_radius_ = impl_->CollisionRadius(
    impl_->joints_[AckermannDrivePrivate::REAR_RIGHT]->GetChild()->GetCollision(id));
  ...
  auto front_right_center_pos = impl_->joints_[AckermannDrivePrivate::FRONT_RIGHT]->
    GetChild()->GetCollision(id)->WorldPose().Pos();
  auto front_left_center_pos = impl_->joints_[AckermannDrivePrivate::FRONT_LEFT]->
    GetChild()->GetCollision(id)->WorldPose().Pos();
  auto rear_right_center_pos = impl_->joints_[AckermannDrivePrivate::REAR_RIGHT]->
    GetChild()->GetCollision(id)->WorldPose().Pos();
  auto rear_left_center_pos = impl_->joints_[AckermannDrivePrivate::REAR_LEFT]->
    GetChild()->GetCollision(id)->WorldPose().Pos();
  ...
  impl_->cmd_vel_sub_ = impl_->ros_node_->create_subscription<geometry_msgs::msg::Twist>(
    ...
    std::bind(&AckermannDrivePrivate::OnCmdVel, impl_.get(), std::placeholders::_1));
  ...
  impl_->update_connection_ = gazebo::event::Events::ConnectWorldUpdateBegin(
    std::bind(&AckermannDrivePrivate::OnUpdate, impl_.get(), std::placeholders::_1));
}

void AckermannDrive::Reset()
{
...
void AckermannDrivePrivate::OnUpdate(const gazebo::common::UpdateInfo & _info)
{
  #ifdef IGN_PROFILER_ENABLE
  IGN_PROFILE("AckermannDrivePrivate::OnUpdate");
  #endif
...
void AckermannDrivePrivate::OnCmdVel(const geometry_msgs::msg::Twist::SharedPtr _msg)
{
...
double AckermannDrivePrivate::CollisionRadius(const gazebo::physics::CollisionPtr & _coll)
{
...
void AckermannDrivePrivate::UpdateOdometryWorld()
{
...
void AckermannDrivePrivate::PublishOdometryTf(const gazebo::common::Time & _current_time)
{
...
void AckermannDrivePrivate::PublishWheelsTf(const gazebo::common::Time & _current_time)
{
...
void AckermannDrivePrivate::PublishOdometryMsg(const gazebo::common::Time & _current_time)
{
  // Set covariance
...
GZ_REGISTER_MODEL_PLUGIN(AckermannDrive)
}  // namespace gazebo_plugins
