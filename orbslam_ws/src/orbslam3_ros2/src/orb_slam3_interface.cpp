...
namespace ORB_SLAM3_Wrapper
{
    ...
    {
        if (!msgIMU) {
            RCLCPP_ERROR(rclcpp::get_logger("ORB-SLAM3"), "handleIMU received NULL IMU message!");
            return;
        }
        bufMutex_.lock();

        if (imuBuf_.size() > 100)  // Limit buffer size
        {
            //std::cout << "IMU buffer too large, dropping oldest IMU data." << std::endl;
            imuBuf_.pop();
        }
        imuBuf_.push(msgIMU);
        ...
        //std::cout << "IMU buffer size after push: " << imuBuf_.size() << std::endl;

    }

    bool ORBSLAM3Interface::trackStereoi(const sensor_msgs::msg::Image::SharedPtr msgL, const sensor_msgs::msg::Image::SharedPtr msgR, Sophus::SE3f &Tcw)
    {
        //std::this_thread::sleep_for(std::chrono::milliseconds(20));
        vector<ORB_SLAM3::IMU::Point> vImuMeas;

        orbAtlas_ = mSLAM_->GetAtlas();
        cv_bridge::CvImageConstPtr cvL;
        cv_bridge::CvImageConstPtr cvR;

        try
        {
            cvL = cv_bridge::toCvShare(msgL);
            cvR = cv_bridge::toCvShare(msgR);
        }
        catch (cv_bridge::Exception &e)
        {
            std::cerr << "cv_bridge exception stereo!" << endl;
            return false;
        }

    {
        ...
        if (imuBuf_.empty())
        {
            std::cout << "Empty buffer IMU .." << std::endl;
        } else {

            std::cout << "buffer IMU full .." << std::endl;

        }

        std::lock_guard<std::mutex> lock(bufMutex_);  // Ensures thread safety


        vImuMeas.clear();

        double cam_time = std::min(
            typeConversions_->stampToSec(msgL->header.stamp),
            typeConversions_->stampToSec(msgR->header.stamp)
        );


        //std::cout << "Camera Time: " << cam_time << std::endl;
        // Ensure at least one IMU measurement is used, even if timestamps are ahead
        std::cout << "IMU buffer size before processing: " << imuBuf_.size() << std::endl;
        imuBuf_temp_ = imuBuf_;
        std::cout << "IMU buffer size before processing: " << imuBuf_temp_.size() << std::endl;

        while (!imuBuf_temp_.empty() && typeConversions_->stampToSec(imuBuf_temp_.front()->header.stamp) <= cam_time)
        {


            double imu_time = typeConversions_->stampToSec(imuBuf_temp_.front()->header.stamp);

            if ((cam_time - imu_time) < 0.02){

            //std::cout << "[DEBUG] IMU time: " << imu_time << ", Camera time: " << cam_time << std::endl;
            //std::cout << "[DEBUG] Condition result: " << (imu_time <= cam_time ? "TRUE" : "FALSE") << std::endl;

            cv::Point3f acc(
                imuBuf_temp_.front()->linear_acceleration.x,
                imuBuf_temp_.front()->linear_acceleration.y,
                imuBuf_temp_.front()->linear_acceleration.z
            );
            cv::Point3f gyr(
                imuBuf_temp_.front()->angular_velocity.x,
                imuBuf_temp_.front()->angular_velocity.y,
                imuBuf_temp_.front()->angular_velocity.z
            );
            vImuMeas.push_back(ORB_SLAM3::IMU::Point(acc, gyr, imu_time));
        ...
        if (vImuMeas.empty())
            std::cout << "vImuMeas is empty ..." << std::endl;
        else
            std::cout << "vImuMeas is not empty ..., size = " << vImuMeas.size() << std::endl;
        ...
        if (vImuMeas.size() > 2)
        {
            ...
            auto Tcw = mSLAM_->TrackStereo(cvL->image, cvR->image, typeConversions_->stampToSec(msgL->header.stamp), vImuMeas);
    ...
    bool ORBSLAM3Interface::trackStereo(const sensor_msgs::msg::Image::SharedPtr msgL, const sensor_msgs::msg::Image::SharedPtr msgR, Sophus::SE3f &Tcw)
    {
        ...
        std::cout << "tracking ..." << std::endl;
        Tcw = mSLAM_->TrackStereo(cvL->image, cvR->image, typeConversions_->stampToSec(msgL->header.stamp));
